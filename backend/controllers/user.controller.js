...
export const followUnfollowUser = async (req, res) => {
  try {
    ...
    if (id === req.user._id.toString()) {
      return res
        .status(400)
        .json({ error: "we can't follow/unfollow yourself" });
    }
...
export const updateUserProfile = async (req, res) => {
  ...
  try {
    ...
    if (currentPassword && newPassword) {
      ...
      if (!isMatch)
        return res.status(400).json({ error: "Cuurent password is incoorect" });

      if (newPassword?.length < 6) {
        return res
          .status(400)
          .json({ error: "Please must be at least 6 characters long" });
      }
